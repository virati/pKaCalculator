...
float return_distance(residue *t1, atom *t2) {
	float td1[3], td2[3];
	residue *parser1 = t1;
	atom *parser2 = t2;
	
	for(int x=0; x < 3; x++) {
		td1[x] = parser1->alpha_c_loc[x];
		td2[x] = parser2->locat[x];
	}
...
float return_distance(residue *t1, residue *t2) {
	float td1[3], td2[3];
	residue *parser1 = t1;
	residue *parser2 = t2;
